...
fn reported_watchdog_elapsed(text: &str) -> std::time::Duration {
    const FIELD: &str = "\"elapsed_seconds\": ";
    let value = text
        .rsplit_once(FIELD)
        .and_then(|(_, value)| value.split([',', '}']).next())
        .expect("WATCHDOG_RESULT must report elapsed_seconds")
        .parse::<f64>()
        .expect("elapsed_seconds must be numeric");
...
fn external_watchdog_rejects_a_runtime_that_cannot_poll_timers() {
    ...
    assert_watchdog_elapsed(
        reported_watchdog_elapsed(&text),
        FIXTURE_WATCHDOG_SECONDS,
        &text,
    );
}
...
fn unjoined_finite_work_keeps_its_receipt_pending_and_skips_cleanup() {
    ...
    assert_unjoined_watchdog(&output);
}
...
fn assert_unjoined_watchdog(output: &Output) {
    let text = diagnostics(output);
    assert_eq!(output.status.code(), Some(1), "{text}");
    assert!(
    ...
    assert_watchdog_elapsed(reported_watchdog_elapsed(&text), 12, &text);
}
