...
| Partial-startup cleanup | Pattern implemented | `take_cleanup`; [SQLx example](../examples/postgres-lifecycle/src/main.rs). Must be explicitly driven, independently of process operation cancellation. A regression proves extracted cleanup completes after dropping the supervisor cancels its operation tokens. |
