...
- [x] Create `batter-core`, move the foundation implementation and tests, and
  reduce `batter` to a compatibility facade plus its existing public examples.
- [x] Move all adapter foundation imports and manifests to `batter-core` while
  preserving direct adapter behavior and the Runlimit → Axum optional edge.
- [x] Route runner, CI, Jig input scopes, documentation, and source maps to the
  new ownership boundary.
- [x] Verify the focused extraction contracts and the complete required
  two-toolchain, rustdoc, HTTP-smoke, and Jig evidence.
- [x] Record the completed acceptance in the Bead and close only the child;
...
Completed 2026-09-18 on the macOS arm64 checkout with Rust 1.98.1 and 1.94.0.
The foundation implementation and tests now live in `batter-core`; `batter` is
the source-compatible re-export facade, and all four adapters depend on core.
The workspace metadata reports seven libraries and two executable packages.

Focused evidence:

- `cargo metadata --format-version 1 --no-deps --locked` reported nine members;
  dependency inspection found no core/adapter normal, build, or dev edge to
  `batter`, while the Runlimit → Axum optional edge remained.
- `cargo test -p batter --all-targets --offline`, core library/tests, all four
  direct adapter suites, `python3 scripts/check_runlimit_features.py`, and
  `python3 scripts/check_scheduling_mutation.py --output-dir <new-directory>`
  passed. The mutation oracle rejected all six mutant replays and accepted all
  six originals.
- `bash scripts/verify.sh` and `RUSTUP_TOOLCHAIN=1.94.0 bash scripts/verify.sh`
  both passed, including workspace tests, compile-fail doctests, Clippy and
  rustdoc. The five HTTP smoke profiles (default, SIGINT, deadline,
  warn-filter, warn-filter+deadline) passed against freshly built
  `batter-axum` binaries under each toolchain.
- `scripts/jig check contract` and `python3 scripts/test_jig_integration.py`
  passed. `scripts/jig work check --plan-id plan_01M2V18X4AB4SCDVCS54BG9TYW`
  passed all five configured targets; current `api:test` receipt is
  `receipt_01M2V2F3SD78PTZACXGRFN3CP6` and the verify gate is fresh.

The live PostgreSQL suites remain ignored or preflight-only because no
disposable database was provisioned. Hosted CI and Linux/macOS claims beyond
this executed macOS arm64 host remain unverified. Task B owns facade adapter
features and Task C owns moving runnable consumers to those features.
