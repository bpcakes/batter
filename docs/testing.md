...
| `install` publishes every catalog description to the recorder it installs, recording reaches it, and a second installation is rejected | [metrics install](../crates/batter-core/tests/metrics_install.rs), a separate binary owning the global recorder |
