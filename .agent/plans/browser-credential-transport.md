...
3. Validate a non-empty application-selected set of browser mutation signals:
   an exact operator-configured Origin or an exact custom request marker, with
   optional Fetch Metadata and JSON-content checks.
...
- [ ] Implement the feature Bead.
- [ ] Publish or otherwise make an exact Batter revision available, only with
  explicit user authorization.
- [ ] Implement the dependent downstream migration at that exact revision.
...
Planning outcome as of 2026-09-15: the feature has a bounded adapter-owned API,
a threat model, explicit non-goals, exact dependency choices, a public behavior
matrix, implementation entrypoints, and a two-node delivery graph. No production
code has been implemented and no runtime claim has been validated.

Update this section during implementation with:

- the final public type and function names;
- any planned behavior intentionally changed and why;
- exact Batter revision made available to the downstream consumer;
- local Linux/macOS and supported-toolchain evidence actually executed;
- the downstream mechanisms removed after adoption;
- remaining limits, especially browser compatibility and proxy behavior.
...
The documentation must say that a custom marker is a browser signal only when
CORS and trusted origins prevent an attacker origin from sending it. Batter does
not configure that boundary.
