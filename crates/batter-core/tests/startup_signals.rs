...
fn failure(
    result: Result<
        batter_core::lifecycle::RunningSupervisor,
        StartupError<InitializationError<RegistrationError>>,
    >,
) -> Arc<batter_core::startup::StartupFailure<InitializationError<RegistrationError>>> {
    match result {
        ...
        match case {
            ...
            "invalid" => assert!(matches!(
                ...
                StartupCause::Failed(InitializationError::Signals(
                    batter_core::lifecycle::SignalRegistrationError::Registration(
                    ...
                    scope
                        ...
                        .register(move || async move {
                            ...
                            *started.lock().unwrap() = Some(Instant::now());
                            ...
                            std::io::stdout().flush().unwrap();
                            std::future::pending::<Result<(), batter_core::BoxError>>().await
                        });
