...
[dev-dependencies]
runledger-test-support = { path = "../batter/runledger/runledger-test-support" }
```

Git consumers select these package names from `https://github.com/bpcakes/batter`
with one immutable Batter revision. Select that same revision for any direct
Batter dependencies. Cargo resolves the native packages and foundation from that
checkout; no consumer patch, sibling Runledger repository, or Git-history check
is required. A source archive also contains the complete local dependency graph.
...
[`smoke` test](https://github.com/bpcakes/runledger/blob/46b5cd085d011e597de9552dfebbed4c19416453/smoke/external-consumer/tests/smoke.rs) is a compile-checked
...
- [Packaged continuation, retry timing, direct recovery, replay, and metrics smoke test](https://github.com/bpcakes/runledger/blob/46b5cd085d011e597de9552dfebbed4c19416453/smoke/external-consumer/tests/smoke.rs)
...
```bash
# Run from the Batter repository root.
cargo check --workspace --all-targets --all-features --locked
cargo test --workspace --all-features --all-targets --locked --no-run
...
Batter's verification runs these native tests together with adapter and facade
checks. See the root [testing guide](../docs/testing.md) for the two supported
Rust toolchains, graph checks and HTTP smoke commands.
...
The root `.cargo/config.toml` sets `SQLX_OFFLINE=true`. The canonical native
cache is `runledger/.sqlx/`; the PostgreSQL and runtime packages carry identical
copies so builds do not depend on the working directory. Canonical migrations
...
```text
runledger/                    # member tree; ../Cargo.toml owns the workspace
├── README.md
├── IMPORT.md                 # source revision and import decisions
├── LICENSE
├── CHANGELOG.md
├── migrations/               # canonical schema source
├── .sqlx/                    # canonical offline query cache
├── docs/                     # native contracts and historical notes
├── runledger-core/
├── runledger-postgres/
├── runledger-runtime/
├── runledger-tui/
