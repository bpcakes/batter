...
| Coordinated observability | Operation/task/cleanup completion and separate HTTP status/outcome/latency. | Useful default tracing and opt-in bounded outcome metrics implemented; the reference application owns an opt-in OTLP metrics exporter with ordered flush; durable trace propagation deferred. |
