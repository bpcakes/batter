...
impl Startup<()> {
    ...
    ///
    /// ```
    /// use batter_core::{cleanup::CleanupBudget,
...
impl<F> ScopedStartup<F> {
    ...
    ///
    /// ```no_run
    /// # use batter_core::{cleanup::CleanupBudget, lifecycle::{ShutdownBudget, Supervisor}, operation::OperationContext, startup::Startup};
    /// # use std::time::Duration;
    /// # fn configured() -> Result<(), batter_core::BoxError> {
    /// let second = Duration::from_secs(1);
    /// let cleanup = CleanupBudget::new(second, second, second)?;
    /// let process = Supervisor::new(ShutdownBudget::new(second, second, second, cleanup)?);
...
///
/// ```compile_fail,E0599
/// # fn restricted(scope: &mut batter_core::startup::ProtectedStartupScope) {
