...
fn failed<E>(
    result: Result<batter_core::lifecycle::RunningSupervisor, StartupError<E>>,
) -> Arc<batter_core::startup::StartupFailure<E>> {
    match result {
...
async fn rejected_reservation_after_prior_acquisition_preserves_registration_and_cleanup() {
    ...
    assert!(matches!(
        report.cause,
        StartupCause::Failed(batter_core::RegistrationError::Duplicate("resource"))
    ));
...
struct SuccessfulDrainDropPanic {
    handle: batter_core::lifecycle::ShutdownHandle,
}
...
async fn shutdown_projection_retains_coordinator_failure_without_formatting_it() {
    ...
    let failure = batter_core::lifecycle::check_shutdown(Err(error.clone())).unwrap_err();
    assert!(!format!("{failure:?} {failure}").contains("private-coordinator-marker"));
    let batter_core::lifecycle::ShutdownFailure::Coordinator(retained) = failure else {
