...
use batter::{
    ...
    startup::{InitializationError, Startup, StartupFailure},
};
...
macro_rules! print_successful_cleanup_records {
    ($cleanup:expr) => {{
        let cleanup = &$cleanup;
        if cleanup.is_success() {
            for record in &cleanup.records {
                match record.name {
                    "secondary.close" => {
                        println!("consumer-cleanup:secondary.close:succeeded");
                    }
                    "database.close" => {
...
fn print_startup_cleanup(error: &(dyn Error + 'static)) {
    let mut current = Some(error);
    while let Some(source) = current {
        if let Some(failure) = source
            .downcast_ref::<StartupFailure<InitializationError<ApplicationFailure>>>()
        {
            print_successful_cleanup_records!(failure.cleanup);
            return;
        }
...
async fn run() -> Result<SharedShutdownReport, ApplicationFailure> {
    ...
    let mut starting = Startup::scoped(
        ...
        move |scope| {
            Box::pin(async move {
                ...
                scope.stage("secondary.acquire").map_err(|_| ApplicationFailure)?;
                let secondary_slot = scope
                    .reserve_cleanup("secondary.close")
                    .map_err(|_| ApplicationFailure)?;
                let (secondary_stop, secondary_stopped) = oneshot::channel();
                let secondary_task = tokio::spawn(async move {
                    let _ = secondary_stopped.await;
                });
                secondary_slot.register(move || async move {
                    let _ = secondary_stop.send(());
                    secondary_task
                        .await
                ...
                scope.stage("application.validation").map_err(|_| ApplicationFailure)?;
                if env::var("CONSUMER_STARTUP_FAILURE").as_deref() == Ok("1") {
                    return Err(ApplicationFailure);
                }
    ...
    let mut running = match starting.wait().await {
        Ok(running) => running,
        Err(error) => {
            print_startup_cleanup(&error);
            return Err(ApplicationFailure);
        }
    };
    running
...
async fn main() -> ExitCode {
    match run().await {
        Ok(report) => {
            print_successful_cleanup_records!(report.cleanup);
