...
| Startup ack/failure, dropped unacknowledged capability, last-owner and waiter drop, retained driver failure; compile-fail rejection of read-only, cloned and repeated startup acknowledgement | [process_ownership.rs](../crates/batter/tests/process_ownership.rs) and `ComponentStartup`/`ShutdownSignal` rustdoc in [capability.rs](../crates/batter/src/lifecycle/capability.rs) |
