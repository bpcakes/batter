...
async fn protected_failure_retains_stage_lifo_cleanup_errors_and_observer_identity() {
    ...
    let batter_core::startup::StartupOutcome::Failed(StartupError::Failed(observed)) =
        observer.wait().await
    else {
...
async fn dropping_protected_owner_during_initialization_drives_cleanup_once() {
    ...
    entered_rx.await.unwrap();
    drop(starting);
    let batter_core::startup::StartupOutcome::Failed(StartupError::Failed(report)) =
        observer.wait().await
    else {
...
async fn protected_owner_and_observer_keep_the_stable_error_type() {
    ...
    let mut owner: batter_core::startup::StartingSupervisor<InitializationError<Failure>> =
        startup.start();
    ...
    let batter_core::startup::StartupOutcome::Failed(StartupError::Failed(observed)) = observed
    else {
