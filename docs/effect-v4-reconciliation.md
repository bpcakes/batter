...
| 6. Observability | [Telemetry](../crates/batter-core/src/telemetry.rs) records outcomes/timing; independent HTTP observation covers assembled routes, probes/fallback and rejection, with explicit response severity and retained correlation under filtering. [Scoped dispatch](../crates/batter-core/src/scoped_dispatch.rs) retains owned-future tracing through polling and destruction. | Opt-in bounded outcome [metrics](../crates/batter-core/src/telemetry/metrics.rs) are implemented; the reference application owns an opt-in OTLP metrics exporter setup and ordered-flush recipe, and no durable trace propagation is implemented.  No global subscriber installation belongs in the library; arbitrary synchronous subscriber failures are not isolated. |
