...
These sources compile through their owning test targets.
...
Native query helpers add five cases in `atomic_live::{query_helpers,query_pool}`
(55 atomic, 118 total live). Actual `query!`, `query_as!` and `query_scalar!`
values cover mapped/scalar/no-result shapes, binds, optional/all results,
RowNotFound, mapper errors, execute semantics, statement counts and closed scopes.
Pool cases cover reuse, error mapping, acquisition/query cancellation and retained
output before cleanup. Paused-clock unit controls enforce parent clamping and a
single deadline across calls without acquisition. The native policy consumer
exercises all four helpers in both phases and recovery modes; Send assertions
cover native and pooled futures. On macOS arm64 with PostgreSQL 18.6, all 118
live cases and five HTTP smokes passed on each of Rust 1.98.1 and 1.94.0.
The full 1.98.1 verify script passed. All 1.94.0 verification commands passed:
an initial native worker container-port failure passed unchanged on focused and
full reruns; the latter passed tests/lint but needed a clean rustdoc rebuild after
a generated search-index read failure. No assertions or timeouts changed.
Required Jig gates pass, including api:test receipt
`receipt_01M35V6V32QTMWZFC5X0ZA6XHA`. The first Jig test command failed with its
cause beyond the truncated preview. A diagnostic compile and plain directory scan
then stalled reading the shared artifact directory; both were stopped. The unchanged
gate passed with `CARGO_TARGET_DIR=/tmp/batter-ywd2-jig-target`. The initial Jig
failure's precise cause remains unconfirmed. The owning plan records logs and retries.

Adapter macro metadata in `crates/batter-sqlx/.sqlx` was generated by SQLx0.9
macros against the disposable PostgreSQL18.6 instance using `SQLX_OFFLINE=false`,
`SQLX_OFFLINE_DIR` set to that absolute directory and `cargo check -p batter-sqlx
--all-features --tests --locked`. Ordinary checks use the committed offline data.
The workspace checks compile against that committed metadata.
...
The workspace matrix includes the package and its runner.
...
runner command is required. All five HTTP executable smoke profiles run at the
end of `scripts/verify.sh` through `scripts/check_http_smokes.py`.
...
bash scripts/verify.sh             # Complete direct verification; no work state.
```
...
There is no separate full-matrix or HTTP rerun after a successful run.
...
verification calls the complete script, so its former standalone HTTP
...
merge groups; manual dispatch uses `origin/master`. Missing exact bases fail;
a new-branch push with an all-zero before SHA uses the empty tree. For an explicit
local comparison use `scripts/jig file-budget check --base <ref>`, or
`--exact-tree <oid> --provenance explicit`. Use
`scripts/jig file-budget check --strict-inventory` to check all governed files.

The unchanged `.jig/file-budget.toml` policy retains the 800-line Rust ceiling,
400/500-line notice/warning thresholds and exact imported-file ceilings. Budget
...
`examples/reference-service/tests/support/provider_effects.rs` (813 lines).
The change-based check retains its existing no-growth rule for such debt; this
workflow removal does not change the limits or refactor those files. The
strict inventory was executed in a disposable copy of the final files because
the checker rejects unstaged tracked deletions in the original worktree.
...
file-budget runtime binary cache. `batter-at-rest-portability.yml` separately
verifies detached source, packaging and an external consumer on Rust 1.94.0;
that CI-only job remains unchanged.

Run script regressions with:

```sh
python3 -m unittest discover -s scripts -p 'test_*.py' -v
```

Verification-entrypoint controls use command fixtures to prove all existing
checks execute, failures propagate, the lockfile remains unchanged and no work
state is created. Source archive controls retain runtime sources and file-budget
configuration but exclude caches and retired plans/receipts. Historical Jig
execution records referenced elsewhere in this document describe prior trees;
they do not define current verification requirements. Deleted plans and state
...
excludes archived reviews under `.agent/reviews`.
...
| Metric catalog labels stay in documented domains; each outcome records once; raw URLs/IDs/errors coalesce; retry attempts and backoff waits are not operations; every retry execution, abandoned admission wait and abandoned cleanup hook records one terminal result, root lifecycle admission records each observed state, unwinding boundaries record `panicked`, and abandoned shutdown drivers record one `dropped` shutdown ([terminal tests](../crates/batter-core/tests/metrics/terminal.rs)); attempts rejected before their factory and clock regression through the production observation path are covered by [unit tests](../crates/batter-core/src/telemetry/metrics/tests.rs); 10,000-rejection storms under a saturated recorder keep identical decisions, one registration per decision and constant series; task/cleanup/shutdown outcomes | [core metrics](../crates/batter-core/tests/metrics.rs) with its [capture recorder](../crates/batter-core/tests/metrics/capture.rs) |
| Retry terminal recording follows destruction of superseded errors and owned factory/classifier/sampler captures, including pending-future destruction; returned-error destruction remains caller-owned | [retry destruction](../crates/batter-core/tests/metrics/retry_destruction.rs), included by the metrics target |
| Shutdown recording follows destruction of the owned stop future; a destructor panic propagates after cleanup, records one `panicked` shutdown and no completed-duration sample | [shutdown destruction](../crates/batter-core/tests/metrics/shutdown_destruction.rs), included by the metrics target |
| `install` publishes every catalog description to the installed recorder inside an active local scope; observations still follow local/global dispatch; rejected recorders are returned without descriptions | [metrics install](../crates/batter-core/tests/metrics_install.rs), a separate binary owning the global recorder |
| Name-table capacity: first admitted names keep slots, later distinct names coalesce to `<overflow>`, series stay within `MAX_SERIES` | [metrics capacity](../crates/batter-core/tests/metrics_capacity.rs), a separate binary owning the process-wide table |
...
the enclosing Cargo command bounds.
...
all-targets pass in `scripts/test_matrix.py` includes it;
...
run through `scripts/verify.sh` in the `scripts` matrix part. macOS CI includes
the portable
...
Runledger patches. Verification includes native sources, package builds, tests,
examples, migration copies and SQLx cache files.
...
execution markers only after successful process exit. Both fixtures are included
in consumer verification.
