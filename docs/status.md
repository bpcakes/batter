...
| Finite process-owned work | Implemented | Synchronous bounded admission, active-scope descendants during drain, typed receipts independent of work lifetime, bounded failure retention. Permanent closure precedes startup/capacity errors, including shutdown before startup and dropped unstarted owners; regressions in `process_ownership.rs`, execution evidence in [validation](validation.md). |
