...
impl Output {
    ...
    pub fn require_final_event(&self, event: &str) -> Result<(), String> {
        // Truncation cannot prove absence. A panic before startup is more
        // informative than missing evidence; a later panic must not erase a
        // complete startup record that still needs timestamp validation.
        self.check_capture_limit()?;
        if self.observed(event).is_some() {
            return Ok(());
        }
        self.check()?;
        Err(format!("child exited before startup evidence: {self:?}"))
    }
